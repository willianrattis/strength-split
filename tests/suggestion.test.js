...
describe("suggestLoads", () => {
  ...
  // count as executed and the shift computation that execOrder:true depends on never fires.
  it("order-aware baseline: an in-order session within 35 days becomes the baseline, unnormalized", () => {
    const sessions = [
      makeSession({ date: "2026-02-10", exercises: [
        makeEntry({ name: "Agachamento", firstSetAt: "2026-02-10T09:05:00", main: [{ weight: 100, reps: 10, repsDone: 10, done: true, doneAt: "2026-02-10T09:05:00" }] }),
        makeEntry({ name: "Supino", firstSetAt: "2026-02-10T09:00:00", main: [{ weight: 60, reps: 10, repsDone: 10, done: true, doneAt: "2026-02-10T09:00:00" }] })
      ] }),
      makeSession({ date: "2026-01-20", exercises: [
        makeEntry({ name: "Supino", firstSetAt: "2026-01-20T09:00:00", main: [{ weight: 55, reps: 10, repsDone: 10, done: true, doneAt: "2026-01-20T09:00:00" }] })
      ] })
    ];
  ...
  it("order-aware baseline: beyond 35 days, the out-of-order session is used with normalization", () => {
    const sessions = [
      ...
      makeSession({ date: "2025-12-01", exercises: [
        makeEntry({ name: "Supino", firstSetAt: "2025-12-01T09:00:00", main: [{ weight: 55, reps: 10, repsDone: 10, done: true, doneAt: "2025-12-01T09:00:00" }] })
      ] })
    ];
    const r = suggestLoads(sessions, "Supino", "kg", undefined, { cfg: mod, execOrder: true });
